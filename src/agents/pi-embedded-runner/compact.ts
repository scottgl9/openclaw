    ...
    try {
      ...
      try {
        ...
        const tryCompactWithModel = async (
          ...
          log.info(
            `[compaction] Attempting compact with ${compactProvider}/${compactModelId}`,
          );
          try {
        ...
        try {
          ...
          if (!compactFallbacks || !Array.isArray(compactFallbacks) || compactFallbacks.length === 0) {
            throw compactErr;
          }
          const triedModels = [`${provider}/${modelId}`];
          ...
          for (const fallbackSpec of compactFallbacks) {
            ...
            try {
              const fbResolved = await resolveModelAsync(fbProvider, fbModelId, agentDir, params.config);
              if (!fbResolved.model) continue;
              let fbRuntimeModel = fbResolved.model;
              const fbAuthProfileId =
                fbProvider.toLowerCase() !== effectiveProvider.toLowerCase()
                  ? undefined
                  : params.authProfileId;
              const { authModel: fbAuthModel, keyInfo: fbKeyInfo, hasExchange: fbHasExchange } =
                await tryAuthForModel(
                  fbRuntimeModel,
                  fbModelId,
